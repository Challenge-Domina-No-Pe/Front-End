        ...
        <div className="flex-1 p-3 overflow-y-auto">
          <style>{`div::-webkit-scrollbar { display: none; }`}</style>
           <nav className="flex flex-col gap-2">
            ...
            <div>
              <div className="flex items-center justify-between">
                ...
                {(isPinned || isMobileOpen) && (
                  <button type="button" onClick={() => setIsCopaOpen(!isCopaOpen)} className="ml-2 p-1">
                    {isCopaOpen ? <ChevronDown /> : <ChevronRight />}
                  </button>
                )}
              </div>
              {isCopaOpen && (isPinned || isMobileOpen) && (
                <div className="ml-6 mt-2 flex flex-col gap-2">
                  <NavLink to="/copa-pab/jogos" className="flex items-center gap-2 p-2 rounded-md hover:bg-purple-700 transition-colors">
                    <SoccerBallIcon /> Jogos
                  </NavLink>
                  <NavLink to="/copa-pab/estatisticas" className="flex items-center gap-2 p-2 rounded-md hover:bg-purple-700 transition-colors">
                    <ChartIcon /> Estatísticas
                  </NavLink>
                  <NavLink to="/copa-pab/fotos" className="flex items-center gap-2 p-2 rounded-md hover:bg-purple-700 transition-colors">
                    <CameraIcon /> Fotos
                  </NavLink>
                </div>
              )}
            ...
            {/* Outros links */}
            <NavLink to="/noticias" style={({ isActive }) => (isActive ? activeLinkStyle : undefined)} className="flex items-center gap-3 p-3 rounded-md hover:bg-purple-800 transition-colors">
              <NewsIcon />
              {(isPinned || isMobileOpen) && <span>Notícias</span>}
            </NavLink>
            <NavLink to="/peneiras" style={({ isActive }) => (isActive ? activeLinkStyle : undefined)} className="flex items-center gap-3 p-3 rounded-md hover:bg-purple-800 transition-colors">
              <PeneirasIcon />
              {(isPinned || isMobileOpen) && <span>Peneiras</span>}
            </NavLink>
            <NavLink to="/escolinhas" style={({ isActive }) => (isActive ? activeLinkStyle : undefined)} className="flex items-center gap-3 p-3 rounded-md hover:bg-purple-800 transition-colors">
              <EscolinhasIcon />
              {(isPinned || isMobileOpen) && <span>Escolinhas</span>}
            </NavLink>
            <NavLink to="/sobre-nos" style={({ isActive }) => (isActive ? activeLinkStyle : undefined)} className="flex items-center gap-3 p-3 rounded-md hover:bg-purple-800 transition-colors">
              <AboutIcon />
              {(isPinned || isMobileOpen) && <span>Sobre</span>}
            </NavLink>
            <NavLink to="/contato" style={({ isActive }) => (isActive ? activeLinkStyle : undefined)} className="flex items-center gap-3 p-3 rounded-md hover:bg-purple-800 transition-colors">
              <ContactIcon />
