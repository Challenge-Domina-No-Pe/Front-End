// src/pages/CopaPAB/Competicao3/Times.jsx
...
/* ----------------- Página ----------------- */
export default function TimesCompeticao3() {
  const [teams, setTeams]       = useState(() => load("c3-teams", FALLBACK_TEAMS));
  const [matches, setMatches]   = useState(() => load("c3-matches", FALLBACK_MATCHES));
  const [rosters, setRosters]   = useState(() => load("c3-rosters", FALLBACK_ROSTERS));
  const [logos, setLogos]       = useState(() => load("c3-logos",   FALLBACK_LOGOS));
  ...
  useEffect(() => save("c3-matches", matches), [matches]);
  useEffect(() => save("c3-rosters", rosters), [rosters]);
  useEffect(() => save("c3-logos", logos), [logos]);
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold text-black mb-4">Times • Competição 3</h1>
          ...
          <div>
            ...
            <HScroller className="mb-2">
              {sel.roster.map((p) => (
                <div
                  key={p.id}
                  className="snap-start w-[360px] shrink-0 rounded-2xl border bg-gray-50 overflow-hidden"
                >
