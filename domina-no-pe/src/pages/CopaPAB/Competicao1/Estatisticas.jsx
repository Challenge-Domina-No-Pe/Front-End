 ...
 * - c1-teams: { A: string[], B: string[], C: string[], D: string[] }
 * - c1-rosters: { [teamName]: Player[] }  (Player tem {nome, numero, posicao, foto, gols, assistencias, ...})
 * - c1-logos: { [teamName]: string(url) } (opcional – usado só pra badge do time)
 */
...
export default function EstatisticasCompeticao1() {
  ...
  useEffect(() => {
    const onStorage = (e) => {
      if (["c1-teams", "c1-rosters", "c1-logos"].includes(e.key)) {
        reload();
      }
    };
