...
/* ----------------- Página ----------------- */
export default function TimesCompeticao2() {
  const [teams, setTeams]       = useState(() => load("c2-teams", FALLBACK_TEAMS));
  const [matches, setMatches]   = useState(() => load("c2-matches", FALLBACK_MATCHES));
  const [rosters, setRosters]   = useState(() => load("c2-rosters", FALLBACK_ROSTERS));
  const [logos, setLogos]       = useState(() => load("c2-logos",   FALLBACK_LOGOS));
  ...
  useEffect(() => save("c2-teams", teams), [teams]);
  useEffect(() => save("c2-matches", matches), [matches]);
  useEffect(() => save("c2-rosters", rosters), [rosters]);
  useEffect(() => save("c2-logos", logos), [logos]);
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold text-black mb-4">Times • Competição 2</h1>
