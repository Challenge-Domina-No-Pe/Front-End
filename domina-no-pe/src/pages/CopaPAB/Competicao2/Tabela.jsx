...
export default function TabelaCompeticao2() {
  ...
  const [matches, setMatches] = useState(() =>
    load("c2-matches", { A: fixtures4(), B: fixtures4(), C: fixtures4(), D: fixtures4() })
  );
  ...
  const addSudden = () => {
    setPenModal((pm) => {
      ...
      return {
        ...pm,
        match: { ...pm.match, pA: ensureLen(pm.match.pA, n), pB: ensureLen(pm.match.pB, n) },
      };
    });
  };
  const removeSudden = () => {
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {!isEdit ? (
          ...
          <div className="flex gap-2">
            <button onClick={saveAll} className="px-4 py-2 rounded-lg bg-green-600 text-white">
              Salvar
            </button>
            <button onClick={cancelAll} className="px-4 py-2 rounded-lg bg-gray-300 text-black">
              Cancelar
            </button>
          </div>
        )}
            ...
            <div key={g} className="bg-purple-700 rounded-xl p-4 shadow-lg text-white">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
                {(isEdit ? draftTeams : teams)[g].map((n, i) => (
                  <input
                    key={i}
                    className={"rounded px-2 py-1 " + (isEdit ? "text-black" : "bg.white/70 text-black/70").replace(".", "/")}
                    readOnly={!isEdit}
                    value={n}
                ...
                {(isEdit ? draftMatches : matches)[g].map((m) => (
                  <div key={m.id} className="bg-purple-800/70 rounded-lg p-3">
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        type="number"
                        className={"w-12 text-center rounded " + (isEdit ? "text-black" : "bg.white/70 text-black/60").replace(".", "/")}
                        readOnly={!isEdit}
                        value={m.ga}
        ...
        {penModal.match && (
          <div>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2 justify-between">
              <div className="flex gap-2">
                <button onClick={clearPens} className="px-3 py-2 rounded-lg bg-gray-200 text-black">
                  Limpar (volta p/ 5)
                </button>
                <button onClick={removeSudden} className="px-3 py-2 rounded-lg bg-gray-200 text-black">
              ...
              <div className="flex gap-2">
                <button onClick={addSudden} className="px-3 py-2 rounded-lg bg-black text-white">
